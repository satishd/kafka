...
public class BrokerHeartbeatManager {
     ...
     * @param isMigratingZkBroker          True if this broker is a ZK broker participating in
     *                                     migration. ZK brokers receive metadata via ZooKeeper
     *                                     rather than the KRaft log directly, so their
     *                                     currentMetadataOffset does not advance past the KRaft
     *                                     log offset; skipping the lowestActiveOffset gate for
     *                                     them avoids a 5-minute controlled-shutdown timeout.
     *
     * @throws IllegalStateException       If the broker is not registered.
        ...
        switch (currentState) {
            ...
            case CONTROLLED_SHUTDOWN:
                ...
                if (isMigratingZkBroker) {
                    log.info("Granting controlled shutdown to ZK migrating broker {} since " +
                        "all its partition leaderships have been moved.", brokerId);
                    return new BrokerControlStates(currentState, SHUTDOWN_NOW);
                }
