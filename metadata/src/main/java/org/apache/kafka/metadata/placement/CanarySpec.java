...
public class CanarySpec {
    static final Predicate<Integer> ALWAYS_FALSE = i -> false;

    private final String canaryPodName;
    private final Predicate<Integer> predicate;

    public CanarySpec(String canaryPodName, double partitionPercentage) {
        this.canaryPodName = canaryPodName;
        if (partitionPercentage > 0.0 && partitionPercentage <= 1.0) {
            final int module = (int) Math.floor(1 / partitionPercentage);
