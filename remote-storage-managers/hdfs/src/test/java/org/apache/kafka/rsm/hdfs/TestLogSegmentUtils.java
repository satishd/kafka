...
public class TestLogSegmentUtils {
    ...
    public static final int OFFSET_INDEX_FILE_SIZE = 100;
    public static final int TIME_INDEX_FILE_SIZE = 50;
    public static final int TXN_INDEX_FILE_SIZE = 300;
    public static final int PRODUCER_SNAPSHOT_FILE_SIZE = 200;
    public static final int LEADER_EPOCH_INDEX_FILE_SIZE = 150;
    ...
    public static LogSegmentData createLogSegmentData(File logDir,
        ...
        Files.write(offsetIndex, TestUtils.randomBytes(OFFSET_INDEX_FILE_SIZE));

        Path timeIndex = new File(logDir, prefix + "." + TIME_INDEX_FILE_NAME).toPath();
        Files.write(timeIndex, TestUtils.randomBytes(TIME_INDEX_FILE_SIZE));

        ByteBuffer leaderEpochIndex = ByteBuffer.wrap(TestUtils.randomBytes(LEADER_EPOCH_INDEX_FILE_SIZE));
        leaderEpochIndex.rewind();

        Path producerSnapshotIndex = new File(logDir, prefix + "." + PRODUCER_SNAPSHOT_FILE_NAME).toPath();
        Files.write(producerSnapshotIndex, TestUtils.randomBytes(PRODUCER_SNAPSHOT_FILE_SIZE));
