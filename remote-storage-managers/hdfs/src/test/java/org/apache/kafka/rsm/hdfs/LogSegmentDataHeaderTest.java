...
class LogSegmentDataHeaderTest {
    ...
    void getDataPosition() throws IOException {
        ...
        for (LogSegmentDataHeader.FileType fileType : LogSegmentDataHeader.FileType.values()) {
            ...
            switch (fileType) {
                case OFFSET_INDEX:
                    length = TestLogSegmentUtils.OFFSET_INDEX_FILE_SIZE;
                    break;
                case TIMESTAMP_INDEX:
                    length = TestLogSegmentUtils.TIME_INDEX_FILE_SIZE;
                    break;
                case LEADER_EPOCH_CHECKPOINT:
                    length = TestLogSegmentUtils.LEADER_EPOCH_INDEX_FILE_SIZE;
                    break;
                case PRODUCER_SNAPSHOT:
                    length = TestLogSegmentUtils.PRODUCER_SNAPSHOT_FILE_SIZE;
                    break;
                case TRANSACTION_INDEX:
                    length = TestLogSegmentUtils.TXN_INDEX_FILE_SIZE;
                    break;
                default:
                    break;
            }
