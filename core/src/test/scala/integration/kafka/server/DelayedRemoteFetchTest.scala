...
class DelayedRemoteFetchTest {
  ...
  def testFetch(): Unit = {
    ...
    val fetchInfo: RemoteStorageFetchInfo = new RemoteStorageFetchInfo(0, false, topicIdPartition, null, null, false)
  ...
  def testRequestExpiry(): Unit = {
    ...
    assertFalse(responses(topicIdPartition).isRemoteFetch)
