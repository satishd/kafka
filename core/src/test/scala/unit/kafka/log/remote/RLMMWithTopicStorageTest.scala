...
class RLMMWithTopicStorageTest extends IntegrationTestHarness {
  ...
  def testRLMMInitializedBeforeOps(): Unit = {
    ...
    try {
      ...
      assertThrows(classOf[KafkaException], () => rlmmWithTopicStorage.earliestLogOffset(tp0, 1))

      assertThrows(classOf[KafkaException], () => rlmmWithTopicStorage.highestLogOffset(tp0, 1))

      assertThrows(classOf[KafkaException], () => rlmmWithTopicStorage.putRemoteLogSegmentData(rlSegMetTp0_0_100))

      assertThrows(classOf[KafkaException], () => rlmmWithTopicStorage.remoteLogSegmentMetadata(tp0, 1L, 0))

      assertThrows(classOf[KafkaException], () => rlmmWithTopicStorage.deleteRemoteLogSegmentMetadata(rlSegMetTp0_0_100))

      assertThrows(classOf[KafkaException], () => rlmmWithTopicStorage.listRemoteLogSegments(tp0, 1))
  ...
  def testRLMMTopicPartitions(): Unit = {
    ...
    try {
      ...
      assertTrue(topicDescription.isInternal)
      assertEquals(partitionCountFromConfig, partitionCountFromRlmm)
      assertEquals(partitionCountFromRlmm, topicDescription.partitions().size())

      // check for whether they have the same no of replicas as it is configured
      topicDescription.partitions().forEach(tp => assertEquals(replicationFactor, tp.replicas().size()))
    } finally {
      mayBeRlmmWithTopicStorage.foreach(x => x.close())
    }
  }
  ...
  def testPutAndGetRemoteLogMetadata(): Unit = {
    ...
    try {
      ...
      assertEquals(rlSegIdTp1_101_300, rlSegIdTp1_150)
      ...
      assertEquals(rlSegIdTp0_0_100, rlSegIdTp0_0)
      ...
      assertEquals(rlSegIdTp0_0_100, rlSegIdTp0_100)
      ...
      assertEquals(rlSegIdTp0_101_200, rlSegIdTp0_101)
      ...
      assertEquals(rlSegIdTp0_101_200, rlSegIdTp0_200)
      ...
      assertEquals(rlSegIdTp0_101_200, rlSegIdTp0_300)
    } finally {
      mayBeRlmmWithTopicStorage.foreach(x => x.close())
    }
    ...
    try {
      ...
      assertEquals(rlSegIdTp0_101_200, remoteLogSegmentId170)
    } finally {
  ...
  def testNonExistingOffsets(): Unit = {
    ...
    try {
      ...
      assertEquals(rlSegIdTp0_10, rlSegIdTp0_200)
    } finally {
      mayBeRlmmWithTopicStorage.foreach(x => x.close())
    }
  }
  ...
  def testDeleteRemoteLogSegment(): Unit = {
    ...
    try {
      ...
      assertEquals(rlSegIdTp0_0_100, rlSegMetTp0_15)
  ...
  def testMultipleRLMMInstanceWorkflow(): Unit = {
    ...
    try {
      ...
      assertEquals(rlSegIdTp0_0_100, rlSegMetTp0_0_1.remoteLogSegmentId());
      val rlSegMatTp0_101_1 = rlmm1.remoteLogSegmentMetadata(tp0, 190, 0);
      assertEquals(rlSegIdTp0_101_200, rlSegMatTp0_101_1.remoteLogSegmentId());

      // check whether these events are received in rlmm2 as it is a follower.
      assertTrue(waitTillReceiveExpected(() => remoteLogSegmentId(rlmm2.remoteLogSegmentMetadata(tp0, 10, 0)), rlSegIdTp0_0_100))
      assertTrue(waitTillReceiveExpected(() => remoteLogSegmentId(rlmm2.remoteLogSegmentMetadata(tp0, 190, 0)), rlSegIdTp0_101_200))
  ...
  def testLeaderFollowerFailover(): Unit = {
    ...
    try {
      ...
      assertEquals(rlSegIdTp0_0_100, rlSegMetTp0_10.remoteLogSegmentId());

      val rlSegMetTp3_140 = rlmm1.remoteLogSegmentMetadata(tp3, 140, 0);
      assertEquals(rlSegIdTp3_101_700, rlSegMetTp3_140.remoteLogSegmentId());
      ...
      assertEquals(rlSegIdTp1_101_300, rlSegIdTp1_180)

      val rlSegIdTp2_300 = remoteLogSegmentId(rlmm2.remoteLogSegmentMetadata(tp2, 300, 0))
      assertEquals(rlSegIdTp2_150_400, rlSegIdTp2_300)

      // check for tp3 messages in rlmm2, but it should not have received
      assertFalse(waitTillReceiveExpected(() => remoteLogSegmentId(rlmm2.remoteLogSegmentMetadata(tp3, 170, 0)), rlSegIdTp3_101_700, 2000L));
      ...
      assertTrue(waitTillReceiveExpected(() => remoteLogSegmentId(rlmm2.remoteLogSegmentMetadata(tp3, 170, 0)), rlSegIdTp3_101_700));

      // add a new segment notification for tp3
      rlmm2.putRemoteLogSegmentData(rlSegMetTp3_701_1900)
      assertTrue(waitTillReceiveExpected(() => remoteLogSegmentId(rlmm2.remoteLogSegmentMetadata(tp3, 720, 0)), rlSegIdTp3_701_1900));

      // rlmm1 should not receive latest tp3 segment notifications as it is not assigned for.
      assertFalse(waitTillReceiveExpected(() => remoteLogSegmentId(rlmm1.remoteLogSegmentMetadata(tp3, 720, 0)), rlSegIdTp3_701_1900, 2000L));
      ...
      assertTrue(waitTillReceiveExpected(() => remoteLogSegmentId(rlmm1.remoteLogSegmentMetadata(tp3, 720, 0)), rlSegIdTp3_701_1900));
