...
import kafka.server.{BrokerTopicStats, RequestLocal, DelayedOperationPurgatory, DelayedRemoteListOffsets, KafkaConfig}
...
class UnifiedLogTest {
  ...
  def testFetchUpToLogEndOffset(): Unit = {
    ...
    (log.logStartOffset until log.logEndOffset).foreach { offset =>
      val batchBaseOffset = batchBaseOffsets.filter(_ <= offset).lastKey
