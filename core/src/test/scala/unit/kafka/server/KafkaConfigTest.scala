  ...
  def testFromPropsInvalid(): Unit = {
    ...
    KafkaConfig.configNames.foreach { name =>
      name match {
        ...
        case ReplicationConfigs.REPLICA_START_OFFSET_STRATEGY_CONFIG => // Ignore string
        ...
        case ServerLogConfigs.LEADER_DEPRIORITIZED_LIST_CONFIG  => // ignore string
        case ServerLogConfigs.NEW_REPLICA_EXCLUDE_LIST_CONFIG => // ignore string
        ...
        case QuotaConfigs.ISR_EXPANSION_RATE_LIMIT_BROKER_LIST_CONFIG => // ignore string
        case QuotaConfigs.ISR_EXPANSION_RATE_LIMIT_CONFIG => assertPropertyInvalid(baseProperties, name, "not_a_number", "0", "-1")
        ...
        case RemoteLogManagerConfig.REMOTE_LOG_INDEX_FILE_CACHE_TTL_MS_PROP => assertPropertyInvalid(baseProperties, name, "not_a_number", -2, -3)
    ...
    ServerTopicConfigSynonyms.TOPIC_CONFIG_SYNONYMS.forEach { case (logConfig, kafkaConfigProp) =>
      logConfig match {
        ...
        case TopicConfig.REMOTE_STORAGE_PROVIDER_CONFIG =>
          // single provider exists, so cannot change it dynamically
        case TopicConfig.REMOTE_STORAGE_PREFETCH_ENABLE_CONFIG =>
          assertDynamic(kafkaConfigProp, true, () => config.remoteLogManagerConfig.isLogRemoteStoragePrefetchEnabled)
