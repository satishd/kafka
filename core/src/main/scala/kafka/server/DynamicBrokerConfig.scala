...
import org.apache.kafka.common.config.{AbstractConfig, ConfigDef, ConfigException, ConfigResource, SaslConfigs, SslConfigs, TopicConfig}
  ...
  private[server] def readDynamicBrokerConfigsFromSnapshot(
    ...
    raftManager.replicatedLog.latestSnapshotId().ifPresent(latestSnapshotId => {
      raftManager.replicatedLog.readSnapshot(latestSnapshotId).ifPresent(rawSnapshotReader => {
        ...
        configHandler.processConfigChanges(ZooKeeperInternals.DEFAULT_STRING, dynamicDefaultConfigs)
...
class DynamicRemoteLogConfig(server: KafkaBroker) extends BrokerReconfigurable with Logging {
  ...
  override def validateReconfiguration(newConfig: KafkaConfig): Unit = {
    newConfig.values.forEach { (k, v) =>
      ...
      if (RemoteLogManagerConfig.REMOTE_LOG_READER_THREADS_PROP.equals(k) ||
          ...
          RemoteLogManagerConfig.REMOTE_LOG_OFFSET_READER_THREADS_PROP.equals(k)) {
        val newValue = v.asInstanceOf[Int]
  ...
  override def reconfigure(oldConfig: KafkaConfig, newConfig: KafkaConfig): Unit = {
    ...
    if (server.remoteLogManagerOpt.nonEmpty) {
      ...
      if (newRLMConfig.remoteLogOffsetReaderThreads() != oldRLMConfig.remoteLogOffsetReaderThreads())
        remoteLogManager.resizeOffsetReaderThreadPool(newRLMConfig.remoteLogOffsetReaderThreads())
    }
  }
...
object DynamicRemoteLogConfig {
  val ReconfigurableConfigs = Set(
    ...
    RemoteLogManagerConfig.REMOTE_LOG_OFFSET_READER_THREADS_PROP,
    RemoteLogManagerConfig.REMOTE_MULTI_PARTITION_FETCH_ENABLE_PROP,
    RemoteLogManagerConfig.REMOTE_MULTI_PARTITION_FETCH_ENABLE_ON_PREFETCH_PROP
  )
}
