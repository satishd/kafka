...
object ReplicaManager {
  ...
  private val timestampMinSupportedVersion: immutable.Map[Long, Short] = immutable.Map[Long, Short](
    ...
    ListOffsetsRequest.EARLIEST_LOCAL_TIMESTAMP -> 6.toShort,
    ListOffsetsRequest.LATEST_TIERED_TIMESTAMP -> 6.toShort
  )
...
class ReplicaManager(val config: KafkaConfig,
  ...
  def fetchOffset(topics: Seq[ListOffsetsTopic],
    ...
    if (delayedRemoteListOffsetsRequired(statusByPartition)) {
      ...
      val delayedRemoteListOffsets = new DelayedRemoteListOffsets(timeout, version, statusByPartition, this, responseCallback)
