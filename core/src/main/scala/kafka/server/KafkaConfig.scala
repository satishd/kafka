...
class KafkaConfig private(doLog: Boolean, val props: util.Map[_, _])
  ...
  def consumptionMetricsClientExcludeList = getList(MetricConfigs.EXCLUDED_CLIENT_PREFIXES_FROM_CONSUMPTION_METRICS_CONFIG)
  ...
  def kafkaSuperUsers = getString(BrokerSecurityConfigs.KAFKA_SUPER_USERS_CONFIG)
  ...
  def deleteTopicEnable = getBoolean(ServerConfigs.DELETE_TOPIC_ENABLE_CONFIG)
  ...
  private[server] def advertisedListeners: Seq[EndPoint] = {
