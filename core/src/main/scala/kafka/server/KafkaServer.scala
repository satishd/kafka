...
class KafkaServer(
  ...
  override def startup(): Unit = {
    try {
      ...
      if (canStartup) {
        ...
        // Persist the hostname => broker id mapping to ZK.
        persistBrokerIdByHostnameInZk(config.brokerId)
  ...
  /**
    * Return brokerId from the hostConfig if it exists.
    * Otherwise a new sequence id generated by updating the broker sequence id path in ZK.
  ...
  private def generateBrokerId(): Int = {
    try {
      zkClient.getHostConfig(CoreUtils.getFirstAdvertisedOrCanonicalHostName(config.advertisedListeners)) match {
        case Some(hostConfig) => hostConfig.brokerId
        case None => zkClient.generateBrokerSequenceId() + config.maxReservedBrokerId
      }
    } catch {
      case e: Exception =>
  ...
  /**
    * Write the hostname => broker id mapping to ZK.
    */
  private def persistBrokerIdByHostnameInZk(brokerId: Int): Unit = {
    zkClient.updateHostConfig(
      CoreUtils.getFirstAdvertisedOrCanonicalHostName(config.advertisedListeners),
      HostConfig(brokerId = brokerId))
  }
}
