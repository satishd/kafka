...
class KafkaServer(
    ...
    try {
      ...
      if (canStartup) {
        ...
        logManager = LogManager(config, initialOfflineDirs,
          ...
          kafkaScheduler, time, brokerTopicStats, logDirFailureChannel, remoteLogManagerConfig)
  ...
  protected def createReplicaManager(isShuttingDown: AtomicBoolean): ReplicaManager = {
    new ReplicaManager(config, metrics, time, Some(zkClient), kafkaScheduler, logManager, remoteLogManager, isShuttingDown, quotaManagers,
