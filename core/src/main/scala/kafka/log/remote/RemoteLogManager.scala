...
import org.apache.kafka.common.log.remote.storage.{LogSegmentData, RemoteLogMetadataManager, RemoteLogSegmentId, RemoteLogSegmentMetadata, RemoteLogStorageManager}
...
class RemoteLogManager(fetchLog: TopicPartition => Option[Log],
  ...
  private def createRemoteLogMetadataManager(): RemoteLogMetadataManager = {
    ...
    val rlmm: RemoteLogMetadataManager = if (classPath != null && !classPath.trim.isEmpty) {
      ...
      new RemoteLogMetadataManagerWrapper(rlmmLoaded, rlmmClassLoader)
    } else {
            ...
            if (index <= 0) {
              ...
              sortedSegments.slice(0, index).foreach { segment =>
                ...
                remoteLogMetadataManager.putRemoteLogSegmentData(id,
                  new RemoteLogSegmentMetadata(id, segment.baseOffset, segment.baseOffset, leaderEpochVal, false, null))
                ...
                remoteLogMetadataManager.putRemoteLogSegmentData(id,
                  new RemoteLogSegmentMetadata(id, segment.baseOffset, segment.baseOffset, leaderEpochVal, true,
      ...
      try {
        val remoteLogStartOffset: Option[Long] =
          if (isLeader()) {
            ...
            remoteLogMetadataManager.setEarliestLogOffset(tp, minOffset)
            Some(minOffset)
          } else {
  ...
  def read(fetchMaxByes: Int, minOneMessage: Boolean, tp: TopicPartition, fetchInfo: PartitionData): FetchDataInfo = {
    ...
    //todo compute start position and end position
    val startPos = 0L
    val endPos = new Optional[Long](0L)
    val is = remoteLogStorageManager.fetchLogSegmentData(remoteLogSegmentMetadata, startPos, endPos)
    val records = MemoryRecords.readableRecords(Utils.readFully(is))
