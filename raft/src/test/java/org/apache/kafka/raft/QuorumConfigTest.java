...
public class QuorumConfigTest {
    ...
    public void testIllegalConfig() {
        assertInvalidConfig(Collections.singletonMap(QuorumConfig.QUORUM_ELECTION_TIMEOUT_MS_CONFIG, "-1"));
        assertInvalidConfig(Collections.singletonMap(QuorumConfig.QUORUM_FETCH_TIMEOUT_MS_CONFIG, "-1"));
        assertInvalidConfig(Collections.singletonMap(QuorumConfig.QUORUM_ELECTION_BACKOFF_MAX_MS_CONFIG, "-1"));
        assertInvalidConfig(Collections.singletonMap(QuorumConfig.QUORUM_LINGER_MS_CONFIG, "-1"));
        assertInvalidConfig(Collections.singletonMap(QuorumConfig.QUORUM_REQUEST_TIMEOUT_MS_CONFIG, "-1"));
        assertInvalidConfig(Collections.singletonMap(QuorumConfig.QUORUM_RETRY_BACKOFF_MS_CONFIG, "-1"));
    }
