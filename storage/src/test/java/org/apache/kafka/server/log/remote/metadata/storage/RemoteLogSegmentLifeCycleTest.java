...
public abstract class RemoteLogSegmentLifeCycleTest {
    ...
    public RemoteLogSegmentLifeCycleTest(TopicIdPartition topicIdPartition) {
        this.topicIdPartition = topicIdPartition;
    }

    public void doTestRemoteLogSegmentLifeCycle() throws Exception {
        ...
        Assertions.assertFalse(remoteLogSegmentMetadata(topicIdPartition, 40, 1).isPresent());
        ...
        Stream.of(0, 1, 2).forEach(epoch -> {
            try {
                Assertions.assertFalse(highestOffsetForEpoch(topicIdPartition, epoch).isPresent());
        ...
        updateRemoteLogSegmentMetadata(segment0Update);
        ...
        RemoteLogSegmentMetadata segment1Metadata = createSegmentUpdateWithState(segment1LeaderEpochs, 101L, 200L,
        ...
        RemoteLogSegmentMetadata segment2Metadata = createSegmentUpdateWithState(segment2LeaderEpochs, 201L, 300L,
                                                                                 RemoteLogSegmentState.COPY_SEGMENT_FINISHED);
        ...
        RemoteLogSegmentMetadata segment3Metadata = createSegmentUpdateWithState(segment3LeaderEpochs, 250L, 400L,
                                                                                 RemoteLogSegmentState.COPY_SEGMENT_FINISHED);
        ...
        for (Map.Entry<EpochOffset, RemoteLogSegmentMetadata> entry : expectedEpochOffsetToSegmentMetadata.entrySet()) {
            ...
            Optional<RemoteLogSegmentMetadata> segmentMetadata = remoteLogSegmentMetadata(topicIdPartition, epochOffset.epoch, epochOffset.offset);
        ...
        updateRemoteLogSegmentMetadata(new RemoteLogSegmentMetadataUpdate(expectedSegment0Metadata.remoteLogSegmentId(),
                                                                          time.milliseconds(), RemoteLogSegmentState.DELETE_SEGMENT_STARTED,
                                                                          BROKER_ID_1));
        Assertions.assertFalse(remoteLogSegmentMetadata(topicIdPartition, 0, 10).isPresent());

        // Update segment with state as DELETE_SEGMENT_FINISHED.
        // It should not be available when we search for that segment.
        updateRemoteLogSegmentMetadata(new RemoteLogSegmentMetadataUpdate(expectedSegment0Metadata.remoteLogSegmentId(),
                                                                          time.milliseconds(), RemoteLogSegmentState.DELETE_SEGMENT_FINISHED,
                                                                          BROKER_ID_1));
        Assertions.assertFalse(remoteLogSegmentMetadata(topicIdPartition, 0, 10).isPresent());
        ...
        for (Map.Entry<Integer, Long> entry : expectedEpochToHighestOffset.entrySet()) {
            ...
            Optional<Long> offset = highestOffsetForEpoch(topicIdPartition, epoch);
        ...
        Optional<Long> highestOffsetForEpoch5 = highestOffsetForEpoch(topicIdPartition, 5);
        Assertions.assertFalse(highestOffsetForEpoch5.isPresent());
    }

    protected abstract void updateRemoteLogSegmentMetadata(RemoteLogSegmentMetadataUpdate segmentMetadataUpdate) throws RemoteStorageException;

    protected abstract Optional<Long> highestOffsetForEpoch(TopicIdPartition topicIdPartition,
                                                            int epoch) throws RemoteStorageException;

    protected abstract Optional<RemoteLogSegmentMetadata> remoteLogSegmentMetadata(TopicIdPartition topicIdPartition,
                                                                                   int leaderEpoch,
                                                                                   long offset) throws RemoteStorageException;

    protected abstract void addCopyInProgressSegment(RemoteLogSegmentMetadata segmentMetadata) throws RemoteStorageException;

    private RemoteLogSegmentMetadata createSegmentUpdateWithState(Map<Integer, Long> segmentLeaderEpochs,
                                                                  long startOffset,
                                                                  long endOffset,
        ...
        addCopyInProgressSegment(segmentMetadata);
        ...
        updateRemoteLogSegmentMetadata(segMetadataUpdate);
