 ...
 *    The purpose of this test is to verify that a new broker {B2} on reassignment of replica and it subsequently gaining
 *    leadership for a remote storage enabled topic, it is able to serve requests for all the valid offsets
 *
 *    - Two segments were uploaded to the remote storage for Ta-p0 and the log-start-offset doesn't start with zero;
 *    - Replica is assigned to a new broker and it is assigned leadership;
 *
 *    Acceptance:
 *    -----------
 *    - The new broker should be able to retrieve records for all valid offsets
 */
public class FollowerFetchRequestOnReassignmentTest extends TieredStorageTestHarness {
    ...
    protected void writeTestSpecifications(TieredStorageTestBuilder builder) {
        ...
        builder
            ...
            // Reassign the partition from broker1 to broker2
            .reassignReplica(topicA, p0, Arrays.asList(broker0, broker2))
            // Ensure the broker2 can become a leader
            .expectLeader(topicA, p0, broker2, true)
            // We should be able to consume all records for the topic with the new broker as leader
            // We should be fetching one record from remote storage and making only one remote fetch request
            .expectFetchFromTieredStorage(broker2, topicA, p0, 1)
